...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  const configuredTitle = NOTION_CONFIG?.TITLE || BLOG.TITLE
  const configuredDescription = NOTION_CONFIG?.DESCRIPTION || BLOG.DESCRIPTION
  const configuredAvatar = NOTION_CONFIG?.AVATAR || BLOG.AVATAR
  const defaultTitle = configuredTitle || 'NotionNext BLOG'
  const defaultDescription =
    configuredDescription || '这是一个由NotionNext生成的站点'
  const defaultPageCover = NOTION_CONFIG?.HOME_BANNER_IMAGE || '/bg_image.jpg'
  const defaultIcon = configuredAvatar || '/avatar.svg'
  ...
  const title = configuredTitle || collection?.name?.[0][0] || defaultTitle
  const description =
